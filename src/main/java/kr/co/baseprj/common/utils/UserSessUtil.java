...
import static kr.co.baseprj.common.base.Constant.SESS_AUTH_GROUP_CD;
import static kr.co.baseprj.common.base.Constant.SESS_USER_ID;
import static kr.co.baseprj.common.base.Constant.SESS_USER_NM;
import static kr.co.baseprj.common.base.Constant.SESS_USER_VO;
...
public class UserSessUtil {
  ...
  public static boolean isLoggedIn(HttpServletRequest request) {
    ...
    return session != null && session.getAttribute(SESS_USER_ID) != null &&
        !"".equals(session.getAttribute(SESS_USER_ID));
  }
  ...
  public static void setUserId(HttpServletRequest request, String id) {
    request.getSession(false).setAttribute(SESS_USER_ID, id);
  }
  ...
  public static void setUserNm(HttpServletRequest request, String userNm) {
    request.getSession(false).setAttribute(SESS_USER_NM, userNm);
  }
  ...
  public static void setAuthGroupCd(HttpServletRequest request, String authGroupCd) {
    request.getSession(false).setAttribute(SESS_AUTH_GROUP_CD, authGroupCd);
  }

  public static void setUserVo(HttpServletRequest request, UserVo userVo) {
    request.getSession(false).setAttribute(SESS_USER_VO, userVo);
  }

  public static String getUserId(HttpServletRequest request) {
    HttpSession session = request.getSession(false);
    if (session != null && (String) session.getAttribute(SESS_USER_ID) != null) {
      return (String) session.getAttribute(SESS_USER_ID);
    } else {
      return null;
    }
  }
  ...
  public static UserVo getUserVo(HttpServletRequest request) {
    ...
    HttpSession session = request.getSession(false);
    if (session != null && session.getAttribute(SESS_USER_ID) != null) {
      // 세션에서 UserVo 객체를 가져와 반환합니다.
      return (UserVo) session.getAttribute(SESS_USER_VO);
    } else {
